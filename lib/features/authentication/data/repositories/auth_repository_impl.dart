...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Either<Failures, UserEntity>> getLoggedInUser() async {
    // debug: AuthRepository.getLoggedInUser started

    try {
      // debug: Getting token from local storage...
      final token = await localDatasource.getAuthToken();

      if (token == null) {
        // debug: No token found in local storage
        return const Left(AuthFailure('No token found'));
      }

      // debug: Token found: ${token.substring(0, 20)}...
      // debug: Checking network connection...

      if (await networkInfo.isConnected) {
        // debug: Network connected, fetching user profile...
        final user = await authRemoteDatasource.getUserProfile(token: token);
        // debug: User profile fetched successfully: ${user.name}
        return Right(user);
      } else {
      ...
      // debug: AuthException occurred: ${e.message}
      // debug: AuthException type: ${e.type}
      // --- PERBAIKAN: Tangani juga token expired di sini ---
      if (e.type == AuthErrorType.tokenExpired) {
        // debug: Token expired, returning TokenExpiredFailure
        return Left(TokenExpiredFailure(e.message));
      }
      // debug: Returning AuthFailure
      return Left(AuthFailure(e.message));
    } on ServerException catch (e) {
      // debug: ServerException occurred: ${e.message}
      return Left(ServerFailure(e.message));
    } on CacheException catch (e) {
      // debug: CacheException occurred: ${e.message}
