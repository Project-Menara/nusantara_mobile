...
class VerifyNumberPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: VerifyNumberView(phoneNumber: phoneNumber),
    );
  }
}
...
class VerifyNumberView extends StatefulWidget {
  final String phoneNumber;
  const VerifyNumberView({super.key, required this.phoneNumber});
    ...
    return BlocListener<OtpBloc, OtpState>(
      listener: (context, state) {
        if (state is OtpVerificationSuccess) {
          showAppFlashbar(context, title: 'Verifikasi Berhasil', message: 'Nomor Anda telah terverifikasi.', isSuccess: true);
          context.push(InitialRoutes.createPin, extra: widget.phoneNumber);
        } else if (state is OtpVerificationFailure) {
          showAppFlashbar(context, title: 'Verifikasi Gagal', message: state.message, isSuccess: false);
        }
      },
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text('Verify Number', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 16)),
          backgroundColor: Colors.white,
          elevation: 0,
          centerTitle: true,
        ),
            ...
            Expanded(
              child: Padding(
                ...
                child: Column(
                  children: [
                    const Spacer(),
                    const Text('Verify Your Number', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
                    const Text('Enter your OTP code below', textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: Colors.grey, height: 1.5)),
                      ...
                      builder: (context, state) {
                        ...
                        return SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed: (_otpCode.length == _otpLength && !isLoading) ? _submitOtp : null,
