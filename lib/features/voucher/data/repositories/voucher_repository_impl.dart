...
class VoucherRepositoryImpl implements VoucherRepository {
  ...
  Future<Either<Failures, List<VoucherEntity>>> getVouchers() async {
    // debug: 🌐 VoucherRepository: Starting getVouchers()
    // debug: 🌐 VoucherRepository: Checking network connection...

    try {
      final isConnected = await networkInfo.isConnected;
      // debug: 🌐 VoucherRepository: Network connected: $isConnected

      // 1. Cek koneksi internet
      if (isConnected) {
        // debug: ✅ VoucherRepository: Network connected, fetching vouchers from API...
        try {
          // 2. Jika terkoneksi, panggil data source untuk mengambil data dari API
          // debug: 🌐 VoucherRepository: Calling remote data source...
          final vouchers = await voucherRemoteDataSource.getVouchers();
          // debug: ✅ VoucherRepository: Successfully got ${vouchers.length} vouchers from remote data source
          // 3. Jika berhasil, kembalikan data yang dibungkus dengan `Right`
          return Right(vouchers);
        } on ServerException catch (e) {
          // debug: ❌ VoucherRepository: Server exception occurred: ${e.message}
          // 4. Jika terjadi error dari server, kembalikan `Left` dengan `ServerFailure`
          return Left(ServerFailure(e.message));
        } catch (e) {
          // debug: ❌ VoucherRepository: Unexpected error occurred: $e
          return Left(ServerFailure('Unexpected error: $e'));
        }
      } else {
        // debug: ❌ VoucherRepository: Network not connected
      ...
      // debug: 💥 VoucherRepository: Exception in getVouchers: $e
  ...
  Future<Either<Failures, ClaimedVoucherEntity>> claimVoucher(
    ...
    // debug: 🎯 VoucherRepository: Starting claimVoucher() for voucher ID: $voucherId
    // debug: 🌐 VoucherRepository: Checking network connection...
    ...
    try {
      ...
      // 1. Cek koneksi internet
      if (isConnected) {
        // debug: ✅ VoucherRepository: Network connected, claiming voucher from API...
        try {
          // 2. Jika terkoneksi, panggil data source untuk claim voucher
          // debug: 🎯 VoucherRepository: Calling remote data source to claim voucher...
          final claimedVoucher = await voucherRemoteDataSource.claimVoucher(
            voucherId,
          );
          // debug: ✅ VoucherRepository: Successfully claimed voucher: ${claimedVoucher.id}
          // 3. Jika berhasil, kembalikan data yang dibungkus dengan `Right`
          return Right(claimedVoucher);
        } on ServerException catch (e) {
          // debug: ❌ VoucherRepository: Server exception occurred while claiming voucher: ${e.message}
          // 4. Jika terjadi error dari server, kembalikan `Left` dengan `ServerFailure`
          return Left(ServerFailure(e.message));
        } catch (e) {
          // debug: ❌ VoucherRepository: Unexpected error occurred while claiming voucher: $e
          return Left(ServerFailure('Unexpected error: $e'));
        }
      } else {
        // debug: ❌ VoucherRepository: Network not connected
      ...
      // debug: 💥 VoucherRepository: Exception in claimVoucher: $e
      return Left(ServerFailure('Error checking network: $e'));
    }
  }
  ...
  getClaimedVouchers() async {
    // debug: 🎟️ VoucherRepository: Starting getClaimedVouchers()
    // debug: 🌐 VoucherRepository: Checking network connection...
    ...
    try {
      ...
      // 1. Cek koneksi internet
      if (isConnected) {
        // debug: ✅ VoucherRepository: Network connected, fetching claimed vouchers from API...
        try {
          // 2. Jika terkoneksi, panggil data source untuk mengambil data dari API
          // debug: 🎟️ VoucherRepository: Calling remote data source to get claimed vouchers...
          final claimedVouchers = await voucherRemoteDataSource
              .getClaimedVouchers();
          // debug: ✅ VoucherRepository: Successfully got ${claimedVouchers.length} claimed vouchers from remote data source
          // 3. Jika berhasil, kembalikan data yang dibungkus dengan `Right`
          return Right(claimedVouchers);
        } on ServerException catch (e) {
          // debug: ❌ VoucherRepository: Server exception occurred while getting claimed vouchers: ${e.message}
          // 4. Jika terjadi error dari server, kembalikan `Left` dengan `ServerFailure`
          return Left(ServerFailure(e.message));
        } catch (e) {
          // debug: ❌ VoucherRepository: Unexpected error occurred while getting claimed vouchers: $e
          return Left(ServerFailure('Unexpected error: $e'));
        }
      } else {
        // debug: ❌ VoucherRepository: Network not connected
      ...
      // debug: 💥 VoucherRepository: Exception in getClaimedVouchers: $e
      return Left(ServerFailure('Error checking network: $e'));
    }
  }
}
